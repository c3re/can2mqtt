...
use can2mqtt::types::{C2MFlags, DirMode};
...
async fn main() {
    ...
    let flags = match C2MFlags::from_args(std::env::args()) {
        Ok((f, _)) => f,
        Err(e) => {
            eprintln!("{e}");
            eprintln!("{}", C2MFlags::description());
            std::process::exit(1);
        }
    };
    start_logging(&flags);
    ...
    info!(
        "Starting can2mqtt version={} {flags}",
    ...
    match get_can_socket(flags.can_interface.clone()) {
        ...
        Ok(c) => {
            cs = c;
            info!("canbus: interface initialized.")
        }
    }
    ...
    run(cs, client, eventloop, abs_path, flags.dir_mode).await;
}
...
async fn run(cs: CanSocket, client: AsyncClient, eventloop: EventLoop, abs_path: PathBuf, dir: DirMode) {
    // Channels
    ...
    let cfg = tokio::spawn(send_config(
        rx_inotify,
        abs_path,
        tx_mqtt.clone(),
        tx_can.clone(),
    ...
    let mqtt_rx = tokio::spawn(mqtt_rx(tx_mqtt.clone(), eventloop, dir)); // this will be where the eventloop runs
    // can
    let can_mng = can_mng(rx_can, &cs, tx_mqtt);
    let can_rx = can_rx(tx_can, &cs, dir);
    ...
    match r {
        Ok(_) => info!(
            "All tasks finished successfully (should not happen, so not so successful as it sounds...)"
        ),
        Err(s) => error!("{s}"),
    };
}
...
async fn send_config(
    ...
    while rx_inotify.recv().await.is_some() {
        match can2mqtt::config::parse(abs_path.to_str().unwrap()) {
            Ok(c) => {
                info!("config: new config loaded.");
            ...
            MQTTMngEvent::Config(new_config) => {
                for topic in config.keys() {
                    match client.unsubscribe(topic).await {
                        Ok(_) => (),
                        Err(e) => {
                            error!("Error unsubscribing {topic}: {e}");
                            return Err(
                                "Error during MQTT unsubscribe, exitting program to circumvent undefined state.",
                            );
                        }
                    }
                }
                for topic in new_config.keys() {
                    match client.subscribe(topic, rumqttc::QoS::AtLeastOnce).await {
                        Ok(_) => (),
                        Err(e) => {
                            error!("Error subscribing {topic}: {e}");
                            return Err(
                                "Error during MQTT subscribe, exitting program to circumvent undefined state.",
                            );
                        }
                    }
                }
                config = *new_config;
                debug!("mqtt: new config active.");
            }
            MQTTMngEvent::RX(p) => {
                match config.get(&p.topic) {
                    // TODO check if we can get rid of that clone with a ref
                    Some(to_can_pair) => {
                        match to_can_pair.convertmode.towards_can(p.payload.clone()) {
                            Ok(cd) => {
                                tx_can
                                    ...
                                    .unwrap();
                                info!(
                                    "CAN <- MQTT ID={} len={} data={:?} convertmode={} topic={} message={:?}",
                                    to_can_pair.id.as_u32(),
                                    cd.len(),
                                    cd,
                                    to_can_pair.convertmode,
                                    p.topic,
                                    p.payload.escape_ascii().to_string()
                                );
                            }
                            Err(e) => warn!(
                                "Error while converting to CAN, Topic: {} convertmode: {}: {}",
...
async fn mqtt_rx(
    tx: Sender<MQTTMngEvent>,
    mut eventloop: EventLoop,
    dir: DirMode,
) -> Result<(), &'static str> {
    // This we use to distinguish between initial connection request (should fail)
    // or intermittent connection failures (should retry)
    loop {
                ...
                if let Event::Incoming(i_event) = e {
                    match i_event {
                        ...
                        Packet::Publish(p) => {
                            ...
                            if dir == DirMode::Bidirectional || dir == DirMode::Mqtt2canOnly {
...
async fn can_mng(
    ...
    while let Some(ev) = rx.recv().await {
        match ev {
            CANMngEvent::Config(new_config) => {
                config = *new_config;
                debug!("canbus: new config active.");
            }
            CANMngEvent::RX(cf) => {
...
async fn can_rx(tx: Sender<CANMngEvent>, cs: &CanSocket, dir: DirMode) -> Result<(), &'static str> {
    loop {
        let cf = match cs.recv().await {
            Ok(cf) => cf,
            Err(_) => return Err("issue while waiting for can frame"),
        };
        // Do unidirectional filtering here
        if dir == DirMode::Bidirectional || dir == DirMode::Can2mqttOnly {
...
fn get_mqtt_connection(settings: String) -> Result<(AsyncClient, EventLoop), String> {
    ...
    let mut mqttoptions = MqttOptions::new(
        format!("can2mqtt v{}", env!("CARGO_PKG_VERSION")),
