...
// Arc because of Send because of tokio::spawn
pub fn get_convertmodes() -> HashMap<String, Arc<dyn Converter>> {
    // create a HashMap with all convertmodes:
    let mut convertmodes: HashMap<String, Arc<dyn Converter>> = HashMap::new();
    let nonecv = Arc::new(NoneConverter::default());
    let bytecolorcv = Arc::new(ByteColor2ColorCodeConverter::default());
    let mymodecv = Arc::new(MyModeConverter::default());
    let pixelcv = Arc::new(PixelBin2Ascii::default());
    let sixteenboolcv = Arc::new(SixteenBool2Ascii::default());
    ...
    for b in [8, 16, 32, 64] {
        ...
        for i in [1, 2, 4, 8] {
            ...
            if (b / 8) * i <= 8 {
                ...
                convertmodes.insert(cv.to_string(), Arc::new(cv));
