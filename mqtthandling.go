...
func MQTTStart(connectString string) {
	...
	if dbg { fmt.Printf("mqtthandler: Starte Verbindung nach: %s\n", connectString) }
	if token := client.Connect(); token.Wait() && token.Error() != nil {
		fmt.Println("mqtthandler: Oh no an error occured...")
		panic(token.Error())
	}
	if dbg { fmt.Printf("mqtthandler: Verbindung erfolgreich hergestellt\n") }
}
...
func MQTTSubscribe(topic string) {
	...
	if dbg { fmt.Printf("mqtthandler: Topic: %s erfolgreich abonniert\n", topic) }
}
...
func MQTTUnsubscribe(topic string) {
	...
	if dbg { fmt.Printf("mqtthandler: Topic: %s erfolgreich unsubscribed\n", topic) }
}

func MQTTPublish(topic string, payload string) {
	if dbg { fmt.Printf("mqtthandler: Sende Nachricht: \"%s\" an Topic: \"%s\"\n", payload, topic) }
	MQTTUnsubscribe(topic)
	token := client.Publish(topic, 0, false, payload)
	token.Wait()
	if dbg { fmt.Printf("mqtthandler: Nachricht erfolgreich gesendet.\n") }
	MQTTSubscribe(topic)
}
